...
export default function Register({ handleClick, goToSignIn }) {
    const [auth, setAuth] = useState({
        username: "",
        password: ""
    });
    const handleSubmit = async () => {
        const res = await chordify_backend.createUser({ username: auth.username });
        console.log(res);
        handleClick();
    };
    ...
    return (<>
            <div data-aos="fade-in" onClick={handleClick} className="fixed inset-0 w-screen h-screen z-20 bg-black bg-opacity-80 flex justify-center items-center sm:py-10 sm:px-20 px-8 py-4 box-border">
                <div data-aos="zoom-in-down" onClick={(e) => e.stopPropagation()} className="relative w-full max-w-md h-full bg-gray-200 rounded-xl flex flex-col px-8 py-12 gap-10 box-border">
                    ...
                    <p className="text-3xl text-black font-semibold self-center">Sign Up to Chordify</p>
