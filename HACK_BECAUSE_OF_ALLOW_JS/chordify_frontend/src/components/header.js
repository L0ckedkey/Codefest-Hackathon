...
import { Bars3Icon, MagnifyingGlassIcon, ShoppingCartIcon, UserCircleIcon, WalletIcon } from "@heroicons/react/24/outline";
import { useState } from "react";
import Logo from "./logo.tsx";
export default function Header() {
    const [isSticked, setIsSticked] = useState(false);
    return (<header className={`sticky z-50 -top-[0.1px] transition-colors duration-150' bg-slate-100 text-black`}>
            <Container>
                <div className={`flex h-[4.25rem] items-center gap-x-2 py-2.5 bg-red-200`}>
                    ...
                    <div className="relative ml-6 hidden h-full flex-1 sm:block">
                        ...
                        <input name="Search" type="text" className={`h-full w-full rounded-xl border bg-white/10 pl-10 hover:bg-white/20 focus:ring-0 ${isSticked ? 'border-slate-200 focus:border-slate-200' : 'border-transparent placeholder:text-slate-200 focus:border-transparent'}`} placeholder="Search"/>
                    </div>
