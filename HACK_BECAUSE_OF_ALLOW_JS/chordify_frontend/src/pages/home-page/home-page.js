            ...
            <div className="relative w-full h-full flex flex-grow flex-col justify-start items-center overflow-x-hidden">
                ...
                <div className='z-0 marquee w-[125%]  h-52 sm:h-40 md:h-60 bg-white -rotate-6 translate-y-1/3 sm:translate-y-1/2 bg-opacity-15'>
                    <div className="marquee__group">
                        {images.map((image, index) => (<img className='p-8' key={index} src={image} alt=''/>))}
                    </div>
                    <div className="marquee__group">
                        {images.map((image, index) => (<img className='p-8' key={index} src={image} alt=''/>))}
                    </div>
                </div>
                <img className='z-10 w-36 h-36 sm:w-56 sm:h-56 rounded-full object-cover self-end  lg:mr-44' data-scroll data-scroll-speed="2" data-scroll-position="top" data-scroll-direction="horizontal" src={banner} alt=""/>
                ...
                <div className="px-4 py-8 sm:px-6 lg:px-8 box-border">
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                        {images.map((image, index) => (<NFTCard key={index} image={image}/>))}
...
function Tabs() {
    return (<div className="pt-8 sm:px-0">
